...
/// within it are used.
class RiveAnimationBytes extends StatefulWidget {
  ...
  const RiveAnimationBytes.asset(
    this.name, {
    this.artboard,
  ...
  const RiveAnimationBytes.network(
    this.name, {
    this.artboard,
    this.animations = const [],
    this.stateMachines = const [],
  ...
  const RiveAnimationBytes.bytes(
    this.byteData, {
    this.artboard,
    this.animations = const [],
    this.stateMachines = const [],
...
class _RiveAnimationState extends State<RiveAnimationBytes> {
  /// Rive controller
  ...
  void initState() {
    ...
    } else if (widget.src == _Source.bytes) {
      Future.value(RiveFile.import(widget.byteData!)).then(_init);
    }
  }
